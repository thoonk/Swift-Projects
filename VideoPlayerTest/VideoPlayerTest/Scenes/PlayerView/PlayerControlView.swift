...
final class PlayerControlView: UIView {
    ...
    var isFullScreen: Bool {
        guard let interfaceOrientation = UIApplication.shared.windows
    ...
    lazy var fullScreenButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage(named: "fullScreen_open"), for: .normal)
        button.tintColor = .white
    ...
    lazy var subtitleButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage(named: "subtitles_off"), for: .normal)
        button.tintColor = .white
...
//    }
    ...
    func player(_ playerView: PlayerView, isPlayerPlaying isPlaying: Bool) {
        if isPlaying {
            ...
            self.pausePlayButton.setImage(systemName: "play.fill", size: 30)
        }
    }
...
private extension PlayerControlView {
    func setupLayout() {
        ...
        fullScreenButton.snp.makeConstraints {
            $0.trailing.equalToSuperview().inset(buttonSpacing)
            $0.bottom.equalTo(videoTimeStackView)
            $0.width.height.equalTo(25)
        }
        
        speedButton.snp.makeConstraints {
            $0.top.trailing.equalToSuperview().inset(buttonSpacing)
            $0.width.height.equalTo(30)
        }
        
        backButton.snp.makeConstraints {
            $0.top.leading.equalToSuperview().inset(buttonSpacing)
            $0.width.height.equalTo(30)
        }
        
        subtitleButton.snp.makeConstraints {
            $0.top.equalTo(speedButton)
            $0.trailing.equalTo(speedButton.snp.leading).offset(-buttonSpacing)
            $0.width.height.equalTo(30)
        }
        
        lockButton.snp.makeConstraints {
            $0.top.equalTo(speedButton)
            $0.trailing.equalTo(subtitleButton.snp.leading).offset(-buttonSpacing)
            $0.width.height.equalTo(30)
        }
    ...
    func handleSubtitle() {
        ...
        if isSubtitleVisible {
            self.subtitleButton.setImage(UIImage(named: "subtitles_on"), for: .normal)
        } else {
            self.subtitleButton.setImage(UIImage(named: "subtitles_off"), for: .normal)
        }
    }
    ...
    func onOrientationChanged() {
        if isFullScreen {
            // landscape 전환
            fullScreenButton.setImage(UIImage(named: "fullScreen_close"), for: .normal)
            ...
            timeLineButton.snp.makeConstraints {
                $0.top.equalTo(speedButton)
                $0.trailing.equalTo(subtitleButton.snp.leading).offset(-buttonSpacing)
                $0.width.height.equalTo(30)
            }
            
            lockButton.snp.remakeConstraints {
                $0.top.equalTo(speedButton)
                $0.trailing.equalTo(timeLineButton.snp.leading).offset(-buttonSpacing)
                $0.width.height.equalTo(30)
            }
        } else {
            // portrait 전환
            fullScreenButton.setImage(UIImage(named: "fullScreen_open"), for: .normal)
            ...
            timeLineButton.snp.removeConstraints()
