...
extension PlayerView: PlayerControlViewDelegate {
    func controlView(_ controlView: PlayerControlView, didButtonTapped button: UIButton) {
        if let type = ButtonType(rawValue: button.tag) {
            ...
            case .back:
                if let parentVC = self.parentViewController as? PlayerVC {
...
//                playerState = .beginToPlay
            ...
            case .menu:
                ...
                if let parentVC = self.parentViewController as? PlayerVC {
                    menuView.transitioningDelegate = parentVC
                    parentVC.present(menuView, animated: true)
                }
            ...
            case .lock:
                ...
                if isLockScreen {
                    if let interfaceOrientation = UIApplication.shared.windows
                        .first(where: { $0.isKeyWindow })?
                        .windowScene?
                        .interfaceOrientation {
